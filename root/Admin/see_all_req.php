...
$query = "SELECT * FROM leave_requests ORDER BY req_date ASC";
        ...
        <div class="container-fluid">
            ...
            <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasNavbar" aria-labelledby="offcanvasNavbarLabel">
                ...
                <div class="offcanvas-body">
                    ...
                    <ul class="fs-5" style="list-style: none;">
                        <li>
                            <a style="text-decoration:none; color:black;" href="leaves_req.php">See Unchecked Requests</a>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    <div class="search-box my-3">
        <form action="" method="POST">
            <div class="input-group mb-3" style="margin-left: 5px; height: 50px;">
                <input type="text" name="searchVal" class="form-control" placeholder="Enter Employee ID" aria-label="Recipient's username" aria-describedby="button-addon2">
                <button class="btn btn-dark" style="width: 10%; margin-right: 10px;" id="button-addon2" name="searchReq" type="submit">Search</button>
                <button name="dispAll" type="submit" class="btn btn-dark" style="width: 10%; margin-right: 10px;" id="button-addon2">Display All</button>

            </div>
        </form>
        <?php
        if (isset($_POST['searchReq'])) {
            unset($_POST['searchReq']);
            $search = $_POST['searchVal'];

            $query2 = "SELECT * FROM leave_requests WHERE (emp_id = '$search')";
            $query_result = mysqli_query($conn, $query2);
            if (@mysqli_num_rows($query_result) <= 0) {
                $query2 = "SELECT * FROM leave_requests WHERE (req_date LIKE '%$search%')";
                $query_result = mysqli_query($conn, $query2);
            }
        }
        ?>
